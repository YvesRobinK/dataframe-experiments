...
class Query_1_2(Ssb):
    ...
    def pandas_execution(self):
        ...
        self.joined = self.joined.loc[self.joined['D_YEARMONTHNUM'] == 199401]
        self.joined = self.joined.loc[(self.joined['LO_DISCOUNT'] <= 6) & (self.joined['LO_DISCOUNT'] >= 4)]
        self.joined = self.joined.loc[(self.joined['LO_QUANTITY'] <= 35) & (self.joined['LO_QUANTITY'] >= 26)]
    ...
    def polars_execution(self):
        ...
        self.joined = self.joined.filter(self.executor.module.col('D_YEARMONTHNUM') == 199401)
        self.joined = self.joined.filter(
            (self.executor.module.col('LO_DISCOUNT') <= 6) & (self.executor.module.col('LO_DISCOUNT') >= 4))
        self.joined = self.joined.filter(
            (self.executor.module.col('LO_QUANTITY') <= 35) & (self.executor.module.col('LO_QUANTITY') >= 26))
    ...
    def dask_execution(self):
        ...
        self.joined = self.joined[self.joined['D_YEARMONTHNUM'] == 199401]
        self.joined = self.joined[(self.joined['LO_DISCOUNT'] <= 6) & (self.joined['LO_DISCOUNT'] >= 4)]
        self.joined = self.joined[(self.joined['LO_QUANTITY'] <= 35) & (self.joined['LO_QUANTITY'] >= 26)]
