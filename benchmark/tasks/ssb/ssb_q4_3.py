...
class Query_4_3(Ssb):
    ...
    def pandas_execution(self):
        ...
        self.joined = self.joined.loc[self.joined['S_NATION'] == 'UNITED STATES']
        self.joined = self.joined.loc[(self.joined['D_YEAR'] == 1997) | (self.joined['D_YEAR'] == 1998)]
        self.joined = self.joined.loc[self.joined['P_CATEGORY'] == 'MFGR#14']
        self.joined['PROFIT'] = self.joined['LO_REVENUE'] - self.joined['LO_SUPPLYCOST']
        self.joined = self.joined[['D_YEAR', 'S_CITY', 'P_BRAND1', 'PROFIT']]
        self.joined = self.joined.groupby(['D_YEAR', 'S_CITY', 'P_BRAND1'], as_index=False).agg({'PROFIT': 'sum'})
        self.joined = self.joined.sort_values(by=['D_YEAR', 'S_CITY', 'P_BRAND1'], ascending=[True, True, True])
    ...
    def polars_execution(self):
        ...
        self.joined = self.joined.filter(self.executor.module.col('S_NATION') == "UNITED STATES")
        self.joined = self.joined.filter(
            (self.executor.module.col('D_YEAR') == 1997) | (self.executor.module.col('D_YEAR') == 1998))
        self.joined = self.joined.filter(
            (self.executor.module.col('P_CATEGORY') == "MFGR#14"))
        self.joined = (
            self.joined.select(self.executor.module.all(), (
                        self.executor.module.col('LO_REVENUE') - self.executor.module.col('LO_SUPPLYCOST')).alias(
                "PROFIT")))
        self.joined = self.joined.select(['D_YEAR', 'S_CITY', 'P_BRAND1', 'PROFIT'])
        self.joined = (
            self.joined.group_by(['D_YEAR', 'S_CITY', 'P_BRAND1'])
            .agg(self.executor.module.col("PROFIT").sum().alias("PROFIT"))
        )
        self.joined = self.joined.sort(by=['D_YEAR', 'S_CITY', 'P_BRAND1'],
    ...
    def dask_execution(self):
        ...
        self.joined = self.joined[self.joined['S_NATION'] == "UNITED STATES"]
        self.joined = self.joined[(self.joined['D_YEAR'] == 1997) | (self.joined['D_YEAR'] == 1998)]
        self.joined = self.joined[(self.joined['P_CATEGORY'] == "MFGR#14")]
        self.joined["PROFIT"] = self.joined["LO_REVENUE"] - self.joined["LO_SUPPLYCOST"]
        self.joined = self.joined[['D_YEAR', 'S_CITY', 'P_BRAND1', 'PROFIT']]
        self.joined = self.joined.groupby(['D_YEAR', 'S_CITY', 'P_BRAND1']).agg({'PROFIT': 'sum'}).reset_index()

        self.joined = self.joined.sort_values(by=['D_YEAR', 'S_CITY', 'P_BRAND1'],
                                              descending=[False, False, False])
    ...
    def vaex_execution(self):
        ...
        self.joined["PROFIT"] = self.joined["LO_REVENUE"] - self.joined["LO_SUPPLYCOST"]
        self.joined = self.joined[['D_YEAR', 'S_CITY', 'P_BRAND1', 'PROFIT']]
        self.joined = self.joined.groupby(by=['D_YEAR', 'S_CITY', 'P_BRAND1'], agg={'PROFIT': self.executor.module.agg.sum('PROFIT')})
        self.joined = self.joined.sort(['D_YEAR', 'S_CITY', 'P_BRAND1'], ascending=[True, True, True])
