...
class Query_4_2(Ssb):
    ...
    def pandas_execution(self):
        ...
        self.joined = self.joined[['D_YEAR', 'S_NATION', 'P_CATEGORY', 'PROFIT']]
        self.joined = self.joined.groupby(['D_YEAR', 'S_NATION', 'P_CATEGORY'], as_index=False).agg({'PROFIT': 'sum'})
        self.joined = self.joined.sort_values(by=['D_YEAR', 'S_NATION', 'P_CATEGORY'], ascending=[True, True, True])
    ...
    def polars_execution(self):
        ...
        self.joined = (
            self.joined.select(self.executor.module.all(),(self.executor.module.col('LO_REVENUE') - self.executor.module.col('LO_SUPPLYCOST')).alias("PROFIT")))
        self.joined = self.joined.select(['D_YEAR', 'S_NATION', 'P_CATEGORY', 'PROFIT'])
        self.joined = (
            self.joined.group_by(['D_YEAR', 'S_NATION', 'P_CATEGORY'])
            .agg(self.executor.module.col("PROFIT").sum().alias("PROFIT"))
        )
        self.joined = self.joined.sort(by=['D_YEAR', 'S_NATION', 'P_CATEGORY'], descending=[False, False, False])
    ...
    def dask_execution(self):
        ...
        self.joined["PROFIT"] = self.joined["LO_REVENUE"] - self.joined["LO_SUPPLYCOST"]
        self.joined = self.joined[['D_YEAR', 'S_NATION', 'P_CATEGORY', 'PROFIT']]
        self.joined = self.joined.groupby(['D_YEAR', 'S_NATION', 'P_CATEGORY']).agg({'PROFIT': 'sum'}).reset_index()
        self.joined = self.joined.sort_values(by=['D_YEAR', 'S_NATION', 'P_CATEGORY'], descending=[False, False, False])
    ...
    def vaex_execution(self):
        ...
        self.joined["PROFIT"] = self.joined["LO_REVENUE"] - self.joined["LO_SUPPLYCOST"]
        self.joined = self.joined[['D_YEAR', 'S_NATION', 'P_CATEGORY', 'PROFIT']]
        self.joined = self.joined.groupby(by=['D_YEAR', 'S_NATION', 'P_CATEGORY'], agg={'PROFIT': self.executor.module.agg.sum('PROFIT')})
        self.joined = self.joined.sort(['D_YEAR', 'S_NATION', 'P_CATEGORY'], ascending=[True, True, True])
